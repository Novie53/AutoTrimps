...
function AT__calcMiSpent(upgrade) {
	var total = 0;
...
function AT_autoMagmiteSpender() {
...
function AT_autoGenerator() {
	if (game.global.world < 230) return;
	
	if (game.global.challengeActive == "Daily" && getPageSetting("AutoGenDC") != 0) {//Daily
		changeGeneratorState(getPageSetting("AutoGenDC"));
	}
	else if (game.global.runningChallengeSquared && getPageSetting("AutoGenC2") != 0) {//C2
		changeGeneratorState(getPageSetting("AutoGenC2"));
	}
	else {
		if (getPageSetting("fuellater") < 0 || game.global.world < getPageSetting("fuellater")) {//Before Fuel
			changeGeneratorState(getPageSetting("beforegen"));
		}
		else if (getPageSetting("fuelend") < 0 || game.global.world < getPageSetting("fuelend")) {//Fuel
			changeGeneratorState(1);
		}
		else {//After Fuel
			changeGeneratorState(getPageSetting("defaultgen"));
		}
	}
}